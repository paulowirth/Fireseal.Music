...
namespace Abstra.Challenge.Tests.Integration.Tracks;

public class TrackIntegrationTests(AbstraApplicationFactory webApplicationFactory, ITestOutputHelper testOutputHelper) 
    : IntegrationTest(webApplicationFactory, testOutputHelper)
{
    ...
    public async Task Tracks_ShouldCreate_ReturnsExpected()
    {
        ...
        var createAlbumResponse =
            await httpClient.PostAsJsonAsync("/abstra/albums", createAlbumRequest);
        ...
        var createTrackResponse =
            await httpClient.PostAsJsonAsync("/abstra/tracks", createTrackRequest);
        
        createTrackResponse.EnsureSuccessStatusCode();

        var trackId = await createTrackResponse.Content.ReadFromJsonAsync<Guid>();

        var getTrackResponse = await httpClient.GetAsync($"/abstra/tracks/{trackId}");
    ...
    public async Task Tracks_ShouldUpdate_ReturnsExpected()
    {
        ...
        var updateTrackResponse =
            await httpClient.PutAsJsonAsync($"/abstra/tracks/{trackId}", updateTrackRequest);
    ...
    public async Task Tracks_ShouldDelete_ReturnsExpected()
    {
        ...
        var deleteTrackResponse =
            await httpClient.DeleteAsync($"/abstra/tracks/{trackId}");
    ...
    public async Task Albums_ShouldList_ReturnsExpected()
    {
        ...
        //Act
        var listResponse = await httpClient.GetAsync("/abstra/tracks");
