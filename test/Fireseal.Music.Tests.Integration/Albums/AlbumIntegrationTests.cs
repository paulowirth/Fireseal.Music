...
namespace Fireseal.Music.Tests.Integration.Albums;

public class AlbumIntegrationTests(FiresealApplicationFactory webApplicationFactory, ITestOutputHelper testOutputHelper) 
    : IntegrationTest(webApplicationFactory, testOutputHelper)
{
    ...
    public async Task Albums_ShouldCreate_ReturnsExpected()
    {
        ...
        //Act
        var createResponse =
            await httpClient.PostAsJsonAsync("/fireseal/albums", createRequest);

        createResponse.EnsureSuccessStatusCode();

        var albumId = await createResponse.Content.ReadFromJsonAsync<Guid>();

        var getResponse = await httpClient.GetAsync($"/fireseal/albums/{albumId}");
    ...
    public async Task Albums_ShouldUpdate_ReturnsExpected()
    {
        ...
        var updateResponse =
            await httpClient.PutAsJsonAsync($"/fireseal/albums/{albumId}", updateRequest);
    ...
    public async Task Albums_ShouldDelete_ReturnsExpected()
    {
        ...
        var deleteResponse = await httpClient.DeleteAsync($"/fireseal/albums/{albumId}");
    ...
    public async Task Albums_ShouldList_ReturnsExpected()
    {
        ...
        //Act
        var response = await httpClient.GetAsync("/fireseal/albums");
